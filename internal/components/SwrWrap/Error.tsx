const ERROR_MAPPING = {
  500: { title: 'Server error', message: 'Oops something went wrong' },
} as Record<string | number, { title: string; message: string; }>

type Props = {
  statusCode: keyof typeof ERROR_MAPPING;
  title?: string;
  message?: string;
}
...
const Error = ({
  ...
  const errorTitle = title || ERROR_MAPPING[statusCode]?.title || 'Unexpected error'
  const errorMessage = message || ERROR_MAPPING[statusCode]?.message || 'sorry please contact with administrator'
